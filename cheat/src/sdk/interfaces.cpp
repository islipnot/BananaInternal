...
namespace sdk
{
	bool init()
	{
		...
		// Resolving functions that get_method cant or that are more direct with this method

		{
			const auto il2cpp_resolve_icall = reinterpret_cast<void* (*)(const char* name)>(GetProcAddress(sdk::internal::hGameAssembly, "il2cpp_resolve_icall"));
			if (!il2cpp_resolve_icall) return false;

			auto resolve_icall = [il2cpp_resolve_icall]<typename t>(t dst, const char* function_name)
			{
				*(void**)dst = il2cpp_resolve_icall(function_name);
				return *(void**)dst != nullptr;
			};

			resolve_icall(&sdk::fn::set_behavior_enabled, "UnityEngine.Behaviour::set_enabled(System.Boolean)");
		}
